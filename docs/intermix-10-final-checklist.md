...
- [x] 24. Explicit production deoptimization implemented for configuration mutation with original definitions restored and compiled singleton/scoped identity transferred. Repeated production loads deoptimize the prior runtime before replacing its fallback bridges, and post-finalization mutation through the builder, a retained manager, or the development container requires recompilation before another production load.
- [x] 25. Diagnostic/compiler metadata separated into a sidecar; the hot artifact contains generated runtime code only.
- [x] 26. Atomic generation, ABI/SHA-256 manifest validation, compiled-environment compatibility checks and prevalidated production loading implemented. Prevalidated loading avoids request-time `hash_file()`.
...
- [x] Public documentation distinguishes development and production runtime selection, build/bootstrap steps, artifact trust, dynamic islands, mutation/redeployment rules and framework ownership boundaries.
